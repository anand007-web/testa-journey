...
interface UserQuizManagerProps {
  onEditQuizQuestions: (quiz: QuizModel) => void;
}

export const UserQuizManager: React.FC<UserQuizManagerProps> = ({ onEditQuizQuestions }) => {
  ...
  const loadData = async () => {
    try {
      ...
      // Filter quizzes to only show the current user's quizzes
      if (user) {
        setQuizzes(loadedQuizzes.filter(q => q.user_id === user.id) as unknown as QuizModel[]);
      }
    } catch (error) {
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-1">
                          ...
                          <Button 
                            variant="ghost" 
                            size="sm"
                            onClick={() => onEditQuizQuestions(quiz)}
